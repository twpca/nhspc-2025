...
ll eval(pair<pll, pll> p) { return abs2(p.X - p.Y); }
...
pair<pll, pll> closest_pair(vector<pll> pts) {
  ...
  auto dc = [&](auto self, int l, int r) -> pair<pll, pll> {
    if (r - l <= 1)
      return make_pair(pll(-1, -1), pll(1e9, 1e9));
    int mid = (l + r) / 2;
    auto ans = min(self(self, l, mid), self(self, mid, r), compAns);
    ll d = eval(ans);
    int x_mid = pts[mid].X;
